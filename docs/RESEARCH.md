...
- Overhead wires are a street feature, not a building-pair feature: dense-city photography (the reference alley, Hong Kong and Bangkok streets) shows spans facade to facade across narrow streets and alleys, several per block, criss-crossing at two rough heights, and nothing across avenues where the span and the traffic clearance defeat them. Utility practice puts a service span at 4-8 m over a pedestrian street, above door and awning height, below the second-floor windows.
- Adopted wire selection: walk each street centerline, cast across it to the first facade on each side, span the pair at a shared tier height; street weight = class (alley, street, road) times narrowness times shortness, so alleys carry several per block and avenues none.
