...
Street `level` is the surface height of an edge: 0 at grade, 8 on a highway deck. Absent reads as 0. A station publishes the volume it occupies: `platform` (plan footprint) with `box` (`bottom` and `top`), and `shafts`, one per entrance in entrance order, each a footprint from `top` down to `bottom` with the `passage` that reaches the platform.
...
- No link enters a station. A link is refused where its solid (its cross section swept along its path) overlaps a platform box, an entrance shaft or a platform passage, both in plan and in height. Height is part of the test, so a bridge crosses over a platform at grade and a tunnel at -4 m passes under one at -12 m; what it stops is a tunnel driven through an entrance shaft, which runs from the street all the way down.
